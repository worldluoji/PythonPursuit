# factory
Python中的工厂模式是一种创建型设计模式，其核心思想是将对象的创建过程封装起来，隐藏具体的创建逻辑，提供一个统一的接口或方法来获取所需的对象实例，而无需直接指定具体的实现类。工厂模式有助于降低代码间的耦合度，提高程序的可扩展性和灵活性。

工厂模式真正的价值在于：
- 解耦：客户端代码不需要知道具体类的实现细节
- 可扩展性：易于添加新产品类型而不修改现有代码
- 维护性：集中管理复杂的创建逻辑
- 符合开闭原则：对扩展开放，对修改关闭

对于简单的对象创建，直接使用 if-else 或字典映射就足够了。当创建逻辑变得复杂或需要频繁扩展时，工厂模式才显示出其真正的优势。

根据不同的应用场景和复杂程度，工厂模式通常有以下几种变体：
