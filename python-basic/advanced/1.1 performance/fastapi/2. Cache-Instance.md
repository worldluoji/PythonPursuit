...
## @asynccontextmanager
有些资源需要在**应用启动时**就初始化好，并在**应用关闭时**优雅地清理。例如：

*   **数据库连接池**：在第一个请求到达前就创建好，所有请求共享。
*   **外部API客户端**：预先配置和认证。
*   **大型模型或配置**：加载到内存，避免每次请求都重复加载。

如果这些操作放在第一个请求中处理，会导致该请求响应极慢；如果放在每个请求中处理，则会造成巨大的资源浪费和性能问题。**FastAPI（通过Starlette）提供的“事件处理器”** 正是用于此场景。

**文档内容**在**主修课程 1.7**中明确列出了“使用应用启动和关闭事件管理资源生命周期”。但**文档未给出具体的代码实现示例**。以下将基于我所掌握的FastAPI/Starlette框架知识进行阐述。

在FastAPI中，这通过 `lifespan` 参数来实现，它接受一个**异步上下文管理器**。其核心模式如下：

```python
from contextlib import asynccontextmanager
from fastapi import FastAPI
from .database import engine, database  # 假设的数据库模块

@asynccontextmanager
async def lifespan(app: FastAPI):
    """
    生命周期事件处理器。
    yield 之前是启动事件代码，之后是关闭事件代码。
    """
    # 启动事件：应用启动前执行
    print(“应用启动中：正在连接数据库...”)
    await database.connect()  # 初始化数据库连接池
    app.state.db_connection = database  # 可将实例存于 app.state 供依赖项使用
    print(“数据库连接池就绪。”)

    yield  # 此处应用正常运行，处理请求

    # 关闭事件：应用关闭后执行
    print(“应用关闭中：正在断开数据库连接...”)
...
1.  **`@asynccontextmanager`**： 这是Python标准库`contextlib`提供的装饰器，用于将一个生成器函数转变为异步上下文管理器。`yield`语句是分割**启动逻辑**与**关闭逻辑**的边界。
2.  **`app.state`**： 这是一个专门用于存储**应用级全局状态**的属性（类型为简单的Python对象）。它是存放数据库连接池、配置客户端等共享资源的理想位置。之后，在路径操作函数或依赖项中，您可以通过 `request.app.state` 来访问这些资源。
3.  **资源注入示例**： 结合依赖注入，您可以优雅地在端点中使用这些预初始化的资源。
    ```python
    from fastapi import Depends, Request

    async def get_database(request: Request):
        “”“依赖项，返回在启动事件中初始化的数据库连接。”“”
...
| 特性 | 事件处理器 | 后台任务 |
| :--- | :--- | :--- |
| **触发时机** | 应用启动/关闭时，各一次。 | 每个匹配的请求结束后。 |
| **主要用途** | 管理**全局、共享、昂贵**的资源生命周期。 | 处理**与单个请求相关**的、可延迟的异步操作。 |
| **执行范围** | 应用级。 | 请求级。 |
| **典型场景** | 连接池初始化/关闭、加载AI模型、创建日志文件。 | 发送邮件、更新缓存、写入日志。 |

### 总结
**事件处理器**让您能够以声明、可靠的方式管理应用级别的资源，确保它们在需要时可用，并在结束时被正确清理。这与**后台任务**一起，构成了FastAPI处理“请求之外”工作的两大支柱：一个面向应用全局，一个面向单个请求。

---


可以通过async engine 创建数据库连接池，并使用 `lifespan` 参数管理其生命周期。
```py
# app/database.py
...
# 从环境变量获取数据库URL，例如：postgresql+asyncpg://user:pass@localhost/db
